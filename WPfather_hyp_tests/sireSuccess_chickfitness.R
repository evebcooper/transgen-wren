...
summary(prop.age)
#negative effect 
#no effect when number of helpers are controlled for

########################################################
#interaction between Dage and prop.legit

#interaction with Dage?
weightWP1<-lmer(WPweightM~prop.legit*Dage+No.hatched+weight.age+pre1992+Jincube+Mage+
                 (1|cohort)+(1|mumID)+(1|domID),data=nests)
summary(weightWP1)
...
#interaction with Dage for both WP and EP weights
weightWP1a<-lmer(weightM~prop.legit*Dage+No.hatched+weight.age+pre1992+Jincube+Mage+
                  (1|cohort)+(1|mumID)+(1|domID),data=nests)
summary(weightWP1a)
#nothing


#interaction with Dage for only EP weights
weightWP1b<-lmer(EPweightM~prop.legit*Dage+No.hatched+weight.age+pre1992+Jincube+Mage+
                   (1|cohort)+(1|mumID)+(1|domID),data=nests)
summary(weightWP1b)
#nothing
...
weightWP2<-lmer(WPweightM~prop.legit+No.hatched+weight.age+pre1992+Jincube+
                  (1|cohort)+(1|domID),data=subset(nests,Dage>1))
summary(weightWP2)
...
#do fathers have less WP offspring as they age?
Dage<-lmer(WPprop~Dage+No.hatched+weight.age+pre1992+Jincube+Mage+
                  (1|cohort)+(1|mum)+(1|domID),data=clutch.beta)
summary(Dage)
